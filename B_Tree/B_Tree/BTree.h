...
template <typename T>
void BNode<T>::DeleteFirst(T &val, BNode<T>* &firstSubTree) //delete the first in the node. returns the value deleted and the first SubTree
{
	val = Key[0];
	firstSubTree = Son[0];
...
template<typename T>
inline T * BTree<T>::search(T key, BNode<T>* root) // searches for a key
{
	if (root == NULL)
		return NULL;
	if (root->searchInNode(key))
		return root->searchInNode(key);
	for (int i = 0; i < root->nsons; i++)
	{
		if (search(key, root->son[i]))
			return(search(key, root->son[i]));
	}
}
...
template <typename T>
int BTree<T>::printSubTreeN(BNode<T> *t, int n) // print a sub tree of a given value
{
	int temp;
	int sum = 0;
	if (t == NULL)
	...
	{
		int i;
		for (i = 0; i < t->nsons && sum < n; i++)
		{
			temp = printSubTreeN(t->Son[i], n);
			sum += temp;
			if (sum >= n) return sum;
			if (i < t->nkeys)
			{
